...
/*展示购物车信息*/
void showcartlist(cman_t *pcart)
{
    /*展示购物车信息(后面加上"+"和"-"按钮)*/
...
float showpaylist(cman_t *cmanager)
{
...
int showpay(cman_t *cmanager, man_t *manager)
{
    ...
    while(1)
    {
        ...
        /*显示商品支付列表*/
        float sum;
        sum = showpaylist(cmanager);  //返回支付总计价格

        /*显示支付二维码*/
        imginfo_t imgcode = {"paycode.jpeg", 500, 0};
        showjpg(&imgcode);

        touchpanel(&tx, &ty);  //检测是否有触碰到屏幕
...
void showcart(cman_t *pCartlist, man_t *manager)
{
    ...
    while(1){
        ...
        showcartlist(pCartlist);  //显示购物车列表
        touchpanel(&tx, &ty);  //等到屏幕点击事件
